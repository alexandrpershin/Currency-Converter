...
val currenciesNameMap = mapOf<String, String>(
...
val currenciesIconMap = mapOf<String, Int>(
    "HKD" to R.raw.hkd,
    "HRK" to R.raw.hrk,
    "HUF" to R.raw.huf,
    "IDR" to R.raw.idr,
...
fun getDummyData(): ArrayList<Currency> {
    val result = ArrayList<Currency>()

    currenciesIconMap.forEach { (code, drawableRes) ->
        val name = currenciesNameMap[code] ?: code
        result.add(
            Currency(
                code,
                name,
                String.format("%.2f", Random.nextDouble(0.1, 999.9)).toDouble(),
                drawableRes
            )
        )
    }
